...
// className is always cast via String() to handle SVGAnimatedString on icon elements.

import { Page } from "playwright";
import { FormData, InputData } from "../../models/PageData";

function safeClass(className: any): string {
  try { return String(className) || ""; } catch { return ""; }
}

export async function extractForms(page: Page, framework: string): Promise<FormData[]> {
  return page.$$eval(
    "form",
    (forms, fw) => {
      function sc(cls: any): string {
        try { return String(cls) || ""; } catch { return ""; }
      }

      return (forms as HTMLFormElement[]).map(form => {
        const inputs = Array.from(
          ...
          if (id) label = document.querySelector(`label[for="${id}"]`)?.textContent?.trim() ?? null;
          if (!label) label = inp.closest("label")?.textContent?.trim() ?? null;
          if (!label) {
            const lby = inp.getAttribute("aria-labelledby");
            if (lby) label = document.getElementById(lby)?.textContent?.trim() ?? null;
          }

          const attrs: Record<string, string> = {};
          for (const a of Array.from(inp.attributes)) attrs[a.name] = a.value;

          let elemFw: string | null = (fw as string) !== "unknown" ? (fw as string) : null;
          if (!elemFw) {
            const cls = sc(inp.className);
        ...
        let formFw: string | null = (fw as string) !== "unknown" ? (fw as string) : null;
        if (!formFw) {
...
export async function extractInputs(page: Page, framework: string): Promise<InputData[]> {
